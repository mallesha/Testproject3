...
neg-hostname-dash-test
    neg-hostname-dash

get_service_device_hostname-neg
    ${headers}=    Create Dictionary    Content-Type    application/yang-data+json
    create session    myssion    ${baseurl_service}    headers=${headers}    auth=${login_auth}
...
update_service_hostname_banner-neg
    ${headers}=    Create Dictionary    Content-Type    application/yang-data+json
    create session    myssion    ${baseurl_service}    headers=${headers}    auth=${login_auth}
...
get_updated_service_device_hostname-neg
    ${headers}=    Create Dictionary    Content-Type    application/yang-data+json
    create session    myssion    ${baseurl_service}    headers=${headers}    auth=${login_auth}
    ${response}=    get request    myssion    /hname-banner-rfs:hname-banner-rfs=${ser-device}/hostname
...
get_updated_service_device_banner-neg
    ${headers}=    Create Dictionary    Content-Type    application/yang-data+json
    create session    myssion    ${baseurl_service}    headers=${headers}    auth=${login_auth}
    ${response}=    get request    myssion    /hname-banner-rfs:hname-banner-rfs=${ser-device}/login-banner
    log    ${response.content}
...
neg-hostname
    ${headers}=    Create Dictionary    Content-Type    application/yang-data+json
    create session    myssion    ${baseurl_service}    headers=${headers}    auth=${login_auth}
    ${json_result}=    run process    python    /home/mallesha/robot/config_files/neg_hostname.py
    log to console    ${json_result.stdout}
    ${response}=    post request    myssion    /    data=${json_result.stdout}
    Create Binary File    response.json    ${response.content}
    ${jsonresponse}=    to json    ${response.content}
    ${data}    get value from json    ${jsonresponse["errors"]["error"][0]}    "error-message"
    log to console    ${data}
...
neg-hostname-dash
    ${headers}=    Create Dictionary    Content-Type    application/yang-data+json
    create session    myssion    ${baseurl_service}    headers=${headers}    auth=${login_auth}
    ${json_result}=    run process    python    /home/mallesha/robot/config_files/neg_hostname_dash.py
    log to console    ${json_result.stdout}
    ${response}=    post request    myssion    /    data=${json_result.stdout}
    Create Binary File    response.json    ${response.content}
    ${jsonresponse}=    to json    ${response.content}
